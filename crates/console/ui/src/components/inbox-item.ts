...
export class InboxItem extends LitElement {
  ...
  override render() {
    ...
    return html`<article class="surface" aria-labelledby="i-${this.task.id}">
      ...
      <pre>${messageText(this.task.status.message)}</pre>
      <form @submit=${this.submit}>
        <label>Your answer or resolution
          <input name="note" required minlength="2" .value=${this.note} @input=${(e: Event) => { this.note = (e.target as HTMLInputElement).value; }} ?disabled=${this.pending}>
        </label>
        <button type="submit" class="primary" ?disabled=${this.pending || this.note.trim().length < 2}>${this.pending ? 'Resolving…' : 'Resolve'}</button>
      </form>
    </article>`;
  }
