...
import { EMPTY, PAGE, SECTION_HELP } from '../copy.js';
...
export class RigPage extends SignalWatcher(LitElement) {
  static override styles = [controls, surface, css`
    ...
    h1 { font-size: 1.6rem; font-weight: 800; font-family: var(--mono); overflow-wrap: anywhere; view-transition-name: var(--vt); }
  ...
  override render() {
    ...
    return html`
      <div class="intro">
        ...
        <p class="page-desc">${PAGE.rig.desc}</p>
      </div>
      ...
      <section aria-labelledby="inbox-h">
        <h2 id="inbox-h">Needs you ${inbox.length === 0 ? nothing : html`<span class="count">${inbox.length}</span>`} <help-tip text=${SECTION_HELP.needsYou} label="About this section"></help-tip></h2>
        ${inbox.length === 0
          ? html`<p class="empty">${EMPTY.needsYou}</p>`
          : html`<div class="grid">${repeat(inbox, (t) => t.id, (t) => html`<inbox-item .task=${t} ?pending=${pending.get().has(t.id) || this.resolving === t.id} .allowed=${can(this.rig, 'resolve')} .reason=${whyNot(this.rig, 'resolve')} @resolve-item=${this.onResolve} @apply-option=${this.onOption}></inbox-item>`)}</div>`}
      </section>
      ${requests.length === 0 ? '' : html`<section aria-labelledby="req-h">
        <h2 id="req-h">Planning <span class="count">${requests.length}</span> <help-tip text=${SECTION_HELP.planning} label="About planning"></help-tip></h2>
      ...
      <section aria-labelledby="epics-h">
        <h2 id="epics-h">Epics <span class="count">${epics.length}</span> <help-tip text=${SECTION_HELP.epics} label="About epics"></help-tip></h2>
        ${epics.length === 0
          ? html`<p class="empty">${EMPTY.epics}</p>`
      ...
      <section aria-labelledby="feed-h">
        <h2 id="feed-h">Live <help-tip text=${SECTION_HELP.liveFeed} label="About the live feed"></help-tip></h2>
        <live-feed rig=${this.rig}></live-feed>
      </section>
      <section aria-labelledby="alerts-h">
        <h2 id="alerts-h">Alerts sent <help-tip text=${SECTION_HELP.alerts} label="About alerts"></help-tip></h2>
