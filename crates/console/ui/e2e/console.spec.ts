...
test('connect, see rigs, open a rig, plan, resolve, stop', async ({ page }) => {
  ...
  await expect(page.locator('attention-panel').getByText('The branch no longer merges')).toBeVisible();
  ...
  await page.locator('epic-card a[href="/rigs/toy/epics/toy-abc"]').click();
  await expect(page).toHaveURL(/\/rigs\/toy\/epics\/toy-abc$/);
  await expect(page.getByRole('heading', { name: 'toy-abc' })).toBeVisible();
  await expect(page.locator('tbody tr')).toHaveCount(3);
  const stop = page.locator('epic-page epic-card footer button');
