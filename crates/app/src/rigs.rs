...
impl HostRig {
    ...
    /// The rig's compose network (`<project>_rig`), which the console joins to reach the ledger.
    #[must_use]
    pub fn network(&self) -> String {
        format!("{}_rig", self.project())
    }
...
mod ledger_tests {
    ...
    #[test]
    fn console_joins_every_rig_network_and_carries_the_ledger_password() {
        let mut reg = HostRegistry::default();
        for n in ["a", "b"] {
            reg = reg
                .add(
                    RigName::try_new(n).expect("name"),
                    "https://x/y.git".into(),
                    "rust".into(),
                    "codex".into(),
                    "main".into(),
        ...
        assert!(yaml.contains("CONSOLE_ALERT_URL: ${CONSOLE_ALERT_URL:-}"));
        assert!(yaml.contains("CONSOLE_ALERT_INTERVAL: ${CONSOLE_ALERT_INTERVAL:-30}"));
