# Plans
Active: `exec-plans/active/` — `docs/exec-plans/active/cross-rig-dependencies.md` (planned; not started). Completed: `exec-plans/completed/`. Debt: `exec-plans/tech-debt-tracker.md`.
Roadmap phases live in `ARCHITECTURE.md §9`; each phase item becomes an exec plan and a beads epic before work starts.

Completed: `exec-plans/completed/harness-engineering-rescaffold.md` (2026-08-29).
Completed: `exec-plans/completed/railway-refactor.md` (2026-08-29).
Completed: `exec-plans/completed/runtime-images.md` (2026-08-29).
Completed: `exec-plans/completed/remote-control.md` (2026-08-29).
Completed: `exec-plans/completed/web-console-v2.md` (2026-08-29).
